...
export const AddEditJob = forwardRef<AddEditJobHandle, AddEditJobProps>(
  function AddEditJob(
    ...
    const [formDrawerMessage, setFormDrawerMessage] = useState<{
      type: "success" | "error";
      text: string;
    } | null>(null);
    ...
    const [benefits, setBenefits] = useState<
      (BenefitItem | { id: string; title: string; sort_order: number; isLocal: true })[]
    >([]);
    ...
    const addBenefit = useCallback(async () => {
      ...
      if (editingJob) {
        try {
          const created = await createBenefit(supabase, {
    ...
    const removeBenefit = useCallback(
      async (id: string) => {
        if (editingJob && !id.startsWith("local-")) {
          try {
            await deleteBenefit(supabase, id);
          } catch {
            /* silently ignore */
          }
        }
        setBenefits((prev) => {
          ...
          if (editingJob) {
            reorderBenefits(
              supabase,
              updated
                .filter((b) => !("isLocal" in b))
                .map((b) => ({ id: b.id, sort_order: b.sort_order })),
            ).catch(() => {});
          }
          return updated;
        });
    ...
    const saveBenefitEdit = useCallback(
      async (id: string) => {
        const title = editingBenefitTitle.trim();
        if (!title) return;
        if (editingJob && !id.startsWith("local-")) {
          try {
            await updateBenefit(supabase, id, { title });
          } catch {
            /* silently ignore */
          }
        }
    ...
    useImperativeHandle(ref, () => ({
      submit: () => {
        const localBenefits = benefits.map((b) => ({
          title: b.title,
    ...
    const handleCreateForm = useCallback(
      async (data: FormBuilderData, fields: FormField[], status: "draft" | "published") => {
            ...
            <Controller
              ...
              render={({ field }) => {
                ...
                const toggle = (val: string) => {
                  ...
                  const sortedIdxs = selected
                    .map((v) => LEVELS.indexOf(v))
                    .sort((a, b) => a - b);
                  if (selected.length === 1) {
                    const [idx] = sortedIdxs;
                    if (Math.abs(valIdx - idx) === 1) {
                      field.onChange([
                        LEVELS[Math.min(idx, valIdx)],
                        LEVELS[Math.max(idx, valIdx)],
                      ]);
                    } else {
                      field.onChange([val]);
                    }
                ...
                {benefits.map((benefit, idx) => (
                  <Stack
                    ...
                    sx={{
                      ...
                      border: "1px solid",
                      borderColor:
                        editingBenefitId === benefit.id ? "primary.main" : "divider",
                      borderRadius: 1.5,
                      ...
                      {editingBenefitId === benefit.id ? (
                        <Tooltip title="Salvează">
                          <IconButton
                            size="small"
                            color="primary"
                            onClick={() => saveBenefitEdit(benefit.id)}
                            aria-label="Salvează beneficiu"
                          >
                    ...
                    <FormControlLabel
                      ...
                      label={
                        <Stack direction="row" alignItems="center" spacing={0.5}>
                          ...
                          >
                            <InfoOutlinedIcon
                              sx={{ fontSize: 15, color: "text.secondary", cursor: "help" }}
                            />
                          </Tooltip>
                        </Stack>
                      }
            ...
            >
              <Button
                ...
                onClick={() => {
                  ...
                  const localBenefits = editingJob
                    ? undefined
                    : benefits.map((b) => ({ title: b.title, sort_order: b.sort_order }));
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    const localBenefits = editingJob
                      ...
                      : benefits.map((b) => ({ title: b.title, sort_order: b.sort_order }));
                    handleSubmit((data) =>
                      onSubmit(data, "published", localBenefits),
